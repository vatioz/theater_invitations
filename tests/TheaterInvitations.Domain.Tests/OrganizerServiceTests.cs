...
public sealed class OrganizerServiceTests
{
    ...
    public async Task Templates_use_distinct_display_names_with_shared_global_addresses()
    {
        ...
        Assert.NotEqual(templates.Single(x => x.FromDisplayName == "Company A").VersionNumber, templates.Single(x => x.FromDisplayName == "Company B").VersionNumber);
    }
    ...
    public async Task Campaign_list_projects_dispatch_counts()
    {
        ...
        var template = new EmailTemplate { Type = EmailTemplateType.InitialInvitation, VersionNumber = 1, FromDisplayName = "Theater", Subject = "Subject", HtmlBody = "<p>Body</p>", PlainTextBody = "Body", State = EmailTemplateState.Active, ContentDigest = "digest", CreatedAtUtc = DateTimeOffset.UtcNow, CreatedBy = "Test" };
    ...
    public async Task Selected_resend_uses_current_party_data_and_records_ineligible_recipients()
    {
        ...
        var template = new EmailTemplate { Type = EmailTemplateType.InitialInvitation, VersionNumber = 1, Name = "Company resend", FromDisplayName = "Theater", Subject = "Invitation", HtmlBody = "<p>Hello</p>", PlainTextBody = "Hello", State = EmailTemplateState.Active, ContentDigest = "resend-digest", CreatedAtUtc = DateTimeOffset.UtcNow, CreatedBy = "Test" };
    ...
    public async Task Reminder_campaign_allows_each_active_pending_party_once()
    {
        ...
        var template = new EmailTemplate { Type = EmailTemplateType.Reminder, VersionNumber = 1, FromDisplayName = "Theater", Subject = "Reminder", HtmlBody = "<p>Reminder</p>", PlainTextBody = "Reminder", State = EmailTemplateState.Active, ContentDigest = "reminder-digest", CreatedAtUtc = DateTimeOffset.UtcNow, CreatedBy = "Test" };

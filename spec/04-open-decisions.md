...
| OD-14 | Which sending domain or subdomain, From name, From address, and Reply-To are approved? | Decided | Communications and IT | Configure the verified From address and monitored organizer Reply-To application-wide in elevated organizer UI. Configure the From display name on each template so company-facing campaigns can differ without changing the Resend-bound address. Template edits preserve identity and invalidate prepared reviews that used older content. Keep API credentials outside the UI. | Phase 4 |
...
| OD-16 | Who approves invitation and reminder content? | Decided | Communications | `Operator` and `ElevatedOperator` may create and edit named templates that become immediately usable; no separate approval gate is required. Editing uses optimistic concurrency. | Phase 4 |
